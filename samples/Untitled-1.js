...
async function followRedirectsTool(input) {
  console.log('followRedirectsTool input:', input);
  const url = input.url;
  try {
    const redirectChain = [];
    ...
    console.log('followRedirectsTool response:', {
      url,
...
async function urlscanTool(input) {
  if (!URLSCAN_API_KEY) {
    console.error('URLSCAN_API_KEY is not configured.');
    return { note: 'no urlscan api key configured' };
  }

  console.log('Starting urlscanTool with input:', input);

  try {
    console.log('Submitting URL to urlscan.io...');
    ...
    const uuid = submit.data.uuid;
    console.log('Polling for results with UUID:', uuid);

    // Poll
    for (let i = 0; i < 12; i++) {
      await wait(2000);
      try {
        console.log(`Polling attempt ${i + 1} for UUID: ${uuid}`);
        //https://urlscan.io/api/v1/result/019da116-21a3-71be-8308-abb856e04b99/
        const res = await axios.get(`https://urlscan.io/api/v1/result/${uuid}/`, { timeout: 15000 });
        console.log(`res.url: ${res.config.url}`);
        if (res && res.data) {
          console.log('Polling successful. Result:', res.data);
          return { url: input.url, uuid, result: res.data };
        }
      } catch (err) {
    ...
    console.warn('Polling timed out for UUID:', uuid);
    return { url: input.url, uuid, note: 'timed out polling urlscan' };
  } catch (err) {
    console.error('Error during urlscanTool execution:', err.message);
    console.error('Full error:', err);
    return { url: input.url, error: String(err) };
  }
}

async function virustotalTool(input) {
  ...
  try {
    ...
    try {
      ...
      return { url: urlStr, vt_lookup: lookup.data };
    } catch (lookupErr) {
      // submit
        ...
        try {
          ...
          if (analysis.data && analysis.data.data && analysis.data.data.attributes && analysis.data.data.attributes.status === 'completed') {
            try {
              ...
              return { url: urlStr, vt_analysis: final.data };
            } catch (e) {
...
async function executeTool(toolName, input) {
  ...
  switch (toolName) {
    case 'follow_redirects':
      return await followRedirectsTool(input);
    // case 'whois':
    //   return await whoisTool(input);
    case 'urlscan':
      return await urlscanTool(input);
    case 'virustotal':
      return await virustotalTool(input);
    case 'ocr':
      return await ocrTool(input);
    default:
...
app.post('/invoke', requireAuth, async (req, res) => {
  try {
    ...
    if (!tool) return res.status(400).json({ error: 'tool missing' });
...
app.post('/scan', requireAuth, async (req, res) => {
  ...
  try {
    ...
    runRequestedTools(scanId).catch(err => {
      console.error('runRequestedTools err', err);
      const r = scans.get(scanId);
      if (r) { r.status = 'error'; r.error = String(err); notifyCallback(r).catch(console.error); }
    });
...
async function runRequestedTools(scanId) {
  console.log(`Starting runRequestedTools for scanId: ${scanId}`);

  const record = scans.get(scanId);
  console.log('Record for runRequestedTools:', record);
  console.log("Array.isArray", Array.isArray(record.body.tools));
  console.log("record.body", record.body);
  console.log("record.body.tools", record.body.tools);
  ...
  console.log(`Tools to run: ${tools}, URLs: ${urls}`);
  const results = {};
      ...
      for (const u of urls) {
        try {
          if (t === 'whois') {
            ...
            results[t].push(await executeTool(t, { domain }));
          } else {
            console.log(`===== Executing tool ${t} for URL: ${u}`);
            const res = await executeTool(t, { url: u });
            console.log(`===== Result for tool ${t} and URL ${u}:`, res);
            results[t].push(res);
          }
        } catch (err) {
          console.error(`Error executing tool ${t} for URL ${u}:`, err);
          results[t].push({ url: u, error: String(err) });
        }
      }
    } else if (t === 'ocr' && record.body.media_id) {
      results.ocr = await executeTool('ocr', { media_id: record.body.media_id });
    } else {
...
async function notifyCallback(record) {
  ...
  try {
    ...
    console.error('notifyCallback failed', err && err.response ? err.response.data : err.message);
    throw err;
  }
}
